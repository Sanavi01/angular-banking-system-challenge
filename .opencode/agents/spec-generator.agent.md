...
2. **Lee el tech stack:** `.opencode/instructions/frontend.instructions.md`
3. **Lee el diccionario de dominio:** `.opencode/copilot-instructions.md`
4. **Lee la plantilla:** `.opencode/skills/generate-spec/spec-template.md` — úsala EXACTAMENTE
5. **Explora el código** para identificar modelos, componentes, servicios y rutas ya existentes (no duplicar)
...
- **`## 2. DISEÑO`** — modelos de datos, endpoints API, diseño frontend (componentes, páginas, servicios, validaciones)
- **`## 3. LISTA DE TAREAS`** — checklists accionables para frontend (componentes, servicios, validaciones), tests y QA
...
- Para este proyecto Angular, el backend es externo (no se implementa).
